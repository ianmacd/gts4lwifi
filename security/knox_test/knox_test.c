...
int run_bksecapp_test(char *buf) {
	char app_name[MAX_APP_NAME_SIZE]={0};
	int qsee_ret = 0, test_result = TEST_PASS, i;
	send_cmd_t *cmd;
	send_cmd_rsp_t *rsp;
	int req_len = 0, rsp_len = 0, len = 0;
	struct qseecom_handle *q_bksecapp_handle = NULL;

	printk(KERN_INFO "knox_test -- %s:\n", __func__);
	snprintf(app_name, MAX_APP_NAME_SIZE, "%s", "bksecapp");

	qsee_ret = qseecom_start_app(&q_bksecapp_handle, app_name, BKSECAPP_QSEE_BUFFER_LENGTH);
	if ( NULL == q_bksecapp_handle ) {
	...
	for(i=0; i<ARRAY_LENGTH(disallowedCmdTable); i++) {
		...
		qseecom_set_bandwidth(q_bksecapp_handle, true);
		qsee_ret = qseecom_send_command(q_bksecapp_handle, cmd, req_len, rsp, rsp_len);
		qseecom_set_bandwidth(q_bksecapp_handle, false);
...
shutdown_and_exit:
	qsee_ret = qseecom_shutdown_app(&q_bksecapp_handle);
	if ( qsee_ret ) {
		printk(KERN_ERR"knox_test--failed to shut down the tzapp.\n");
		test_result = TEST_FAIL;
	}
	else
		q_bksecapp_handle = NULL;
...
static ssize_t knox_test_read(struct file *fp, char __user *buf, size_t len, loff_t *off)
{
	...
	count = run_bksecapp_test(ret_buf);
...
static int __init knox_test_init(void)
{
	...
	dent = debugfs_create_file("bksecapp", S_IRUGO, dent_dir, NULL, &knox_test_proc_fops);
	if (dent == NULL) {
	...
	printk(KERN_INFO"knox_test -- %s: registered /d/knox_test/bksecapp interface\n", __func__);
	return 0;
exit:
