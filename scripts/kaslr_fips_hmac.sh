...
# Modified on:	17 Jan	2017  by Rohan Bhutkar (r1.bhutkar@samsung.com) /* For msm8998 platform */
...
kaslr_patch () {
	index=$1
	./kaslr_fips $vmlinux_var $reloc_start_addr $reloc_end_addr $dynsym_addr $index $first_crypto_rodata $last_crypto_rodata 0 0  $2 $3
	retval=$?
	if [ $retval -ne 0 ]; then
...
hmac_patch() {
	...
	# Note that this must be different for each iteration if vmlinux is patched correctly each time.
...
rela_va_addr=`cat vmlinux.elf|grep -w '.rela'|awk '{print $4}'`
rela_file_addr=`cat vmlinux.elf|grep -w '.rela'|awk '{print $5}'`
rela_va_to_file=`expr $((16#$rela_va_addr)) - $((16#$rela_file_addr))`

# the loop
for index in {0..63}
do
	kaslr_patch $index $va_to_file $rela_va_to_file
