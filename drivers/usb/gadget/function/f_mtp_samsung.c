...
//	DEBUG_MTPR("[%s] and count = (%d)\n", __func__, count);
	if (the_mtpg  == NULL){
		r = -ENOENT;
		printk(KERN_ERR "[%s]\t%d:mtpg == NULL , but mtpg_read is called  r=%d\n",
					__func__, __LINE__, r);
		goto fail;
	}
  ...
  struct mtp_event        event;
	...
	case MTP_ONLY_ENABLE:
		...
		if (dev->cdev && dev->cdev->gadget) {
			...
			printk(KERN_ERR "usb: [%s:%d] B4 disconectng gadget\n",
							__func__, __LINE__);
			msleep(20);
			usb_gadget_connect(cdev->gadget);
			printk(KERN_ERR "usb: [%s:%d] after usb_gadget_connect\n",
							__func__, __LINE__);
		}
		status = 10;
