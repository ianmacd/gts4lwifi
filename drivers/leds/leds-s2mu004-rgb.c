...
static int s2mu004_rgb_probe(struct platform_device *pdev)
{
    ...
    ddata->dev = device_create(sec_class, NULL, 0, ddata, "led");
    if (IS_ERR(ddata->dev)) {
...
err_sysfs_create:
    device_destroy(sec_class, ddata->dev->devt);
    return ret;
}
...
static int __init s2mu004_fled_init(void)
{
    printk("=====>[%s,%d]\n", __func__, __LINE__);
    return platform_driver_register(&s2mu004_fled_driver);
}

void __exit s2mu004_fled_exit(void)
{
    printk("=====>[%s,%d]\n", __func__, __LINE__);
    platform_driver_unregister(&s2mu004_fled_driver);
}
module_init(s2mu004_fled_init);
module_exit(s2mu004_fled_exit);
