...
#define MODEL_NAME               "SX9320_WIFI_SUB"
#define MODULE_NAME              "grip_sensor_wifi_sub"
...
static int sx9320_i2c_write(struct sx9320_p *data, u8 reg_addr, u8 buf)
{
	...
	if (ret < 0)
		pr_err("[SX9320_WIFI_SUB]: %s - i2c write error %d\n",
			__func__, ret);
...
static int sx9320_i2c_read(struct sx9320_p *data, u8 reg_addr, u8 *buf)
{
	...
	if (ret < 0)
		pr_err("[SX9320_WIFI_SUB]: %s - i2c read error %d\n",
			__func__, ret);
...
static void sx9320_initialize_register(struct sx9320_p *data)
{
	...
	for (idx = 0; idx < (int)(sizeof(setup_reg) >> 1); idx++) {
		...
		pr_info("[SX9320_WIFI_SUB]: %s - Write Reg: 0x%x Value: 0x%x\n",
...
static void sx9320_initialize_chip(struct sx9320_p *data)
{
	...
	if (cnt >= 10)
		pr_err("[SX9320_WIFI_SUB]: %s - s/w reset fail(%d)\n", __func__, cnt);
...
static void sx9320_send_event(struct sx9320_p *data, u8 state)
{
	if (state == ACTIVE) {
		data->state = ACTIVE;
		pr_info("[SX9320_WIFI_SUB]: %s - button touched\n", __func__);
	} else {
		data->state = IDLE;
		pr_info("[SX9320_WIFI_SUB]: %s - button released\n", __func__);
	}

	if (data->skip_data == true) {
		pr_info("[SX9320_WIFI_SUB]: %s - skip grip event\n", __func__);
		return;
	}
...
static void sx9320_get_data(struct sx9320_p *data)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - Capmain: %d, Useful: %d, avg: %d, diff: %d, Offset: %u\n",
...
static int sx9320_set_mode(struct sx9320_p *data, unsigned char mode)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - change the mode : %u\n", __func__, mode);
	return ret;
}
...
static void sx9320_read_ch_interrupt(struct sx9320_p *data, u8 status)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - ch1:%d, ch2:%d\n",
...
static void sx9320_set_enable(struct sx9320_p *data, int enable)
{
	...
	if (enable == ON) {
		...
		pr_info("[SX9320_WIFI_SUB]: %s - disable\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoul(buf, 10, &val)) {
		pr_err("[SX9320_WIFI_SUB]: %s - Invalid Argument\n", __func__);
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - Register(0x%2x) data(0x%2x)\n",
			__func__, regist, val);
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - Freq : 0x%x\n", __func__, data->freq);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - normal threshold %lu\n", __func__, val);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - handle = %d\n", __func__, handle);
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - called\n", __func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - kstrtou8 failed.(%d)\n", __func__, ret);
		return count;
	}
	...
	if (onoff == 0) {
		...
		pr_err("[SX9320_WIFI_SUB]: %s - unknown value %d\n", __func__, onoff);
	}

	mutex_unlock(&data->read_mutex);

	pr_info("[SX9320_WIFI_SUB]: %s - %d\n", __func__, onoff);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - new_value = %u old_value = %d\n",
...
static void sx9320_touch_process(struct sx9320_p *data, u8 flag)
{
	...
	if (data->state == IDLE) {
		if (status & (PHX_STATUS_REG << data->phen))
			...
			pr_info("[SX9320_WIFI_SUB]: %s - already released\n",
				__func__);
	} else {
		if (!(status & (PHX_STATUS_REG << data->phen)))
			sx9320_send_event(data, IDLE);
		else
			pr_info("[SX9320_WIFI_SUB]: %s - still touched\n",
				__func__);
	}
}
...
static void sx9320_process_interrupt(struct sx9320_p *data)
{
	...
	if (flag & IRQ_PROCESS_CONDITION)
		...
		pr_info("[SX9320_WIFI_SUB]: %s interrupt generated but skip\n",
			__func__);
}
...
static void sx9320_irq_work_func(struct work_struct *work)
{
	...
	if (sx9320_get_nirq_state(data) == 0)
		...
		pr_err("[SX9320_WIFI_SUB]: %s - nirq read high %d\n",
...
static irqreturn_t sx9320_interrupt_thread(int irq, void *pdata)
{
	...
	if (sx9320_get_nirq_state(data) == 1) {
		pr_err("[SX9320_WIFI_SUB]: %s - nirq read high\n", __func__);
	} else {
...
static int sx9320_setup_pin(struct sx9320_p *data)
{
	...
	ret = gpio_request(data->gpio_nirq, "SX9320_WIFI_SUB_nIRQ");
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - gpio %d request failed (%d)\n",
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - failed to set gpio %d(%d)\n",
			__func__, data->gpio_nirq, ret);
...
static int sx9320_read_setupreg(struct device_node *dnode, char *str, u8 *val)
{
	...
	if (!ret)
		*val = (u8)temp_val;
		...
		pr_err("[SX9320_WIFI_SUB]: %s - %s: property read err 0x%2x (%d)\n",
...
static int sx9320_parse_dt(struct sx9320_p *data, struct device *dev)
{
	...
	data->gpio_nirq = of_get_named_gpio_flags(node,
		"sx9320_wifi_sub,nirq-gpio", 0, &flags);
	if (data->gpio_nirq < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - get gpio_nirq error\n", __func__);
		return -ENODEV;
	}
	...
	if (sx9320_read_setupreg(node, SX9320_NORMALTHD_TA,
				&data->normal_th_ta)) {
		pr_info("[SX9320_WIFI_SUB]: %s - dt has no TA thd \n", __func__);
		data->normal_th_ta = data->normal_th;
	}

	pr_info("[SX9320_WIFI_SUB]: afeph0:%x, afeph1:%x, afeph2:%x, afeph3:%x\n",
		...
		unsigned long action, void *data)
{
	...
	if (pdata->init_done == ON) {
		...
		case USB_STATUS_NOTIFY_DETACH:
			...
			break;
		default:
			pr_info("[SX9320_WIFI_SUB]: %s - DRP type : %d\n",
				...
				unsigned long action, void *data)
{
	...
	case ATTACHED_DEV_AFC_CHARGER_9V_MUIC:
		if (action == MUIC_NOTIFY_CMD_ATTACH)
			pr_info("[SX9320_WIFI_SUB]: %s - TA/USB insert\n", __func__);
		else if (action == MUIC_NOTIFY_CMD_DETACH)
			pr_info("[SX9320_WIFI_SUB]: %s - TA/USB remove\n", __func__);

		if (grip_data->init_done == ON)
			sx9320_set_offset_calibration(grip_data);
		else
			pr_info("[SX9320_WIFI_SUB]: %s - not initialized\n", __func__);
		break;
	default:
		break;
	}

	pr_info("[SX9320_WIFI_SUB]: %s dev=%d, action=%lu\n",
...
static int sx9320_check_chip_id(struct sx9320_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: whoami[0x%x] read failed %d\n", value, ret);
		return ret;
	}
	...
	case 0x21:
		return 0;
	default:
		pr_err("[SX9320_WIFI_SUB]: invalid whoami(%x)\n", value);
		return -1;
	}
}
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("[SX9320_WIFI_SUB]: %s - Probe Start!\n", __func__);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("[SX9320_WIFI_SUB]: %s - i2c_check_functionality error\n",
			__func__);
		goto exit;
	}
	...
	if (data == NULL) {
		pr_err("[SX9320_WIFI_SUB]: %s - kzalloc error\n", __func__);
		ret = -ENOMEM;
		goto exit_kzalloc;
	}
	...
	wake_lock_init(&data->grip_wake_lock,
		WAKE_LOCK_SUSPEND, "grip_wifi_sub_wake_lock");
	mutex_init(&data->read_mutex);

	ret = sx9320_parse_dt(data, &client->dev);
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - of_node error\n", __func__);
		ret = -ENODEV;
		goto exit_of_node;
	}
	...
	if (ret) {
		pr_err("[SX9320_WIFI_SUB]: %s - could not setup pin\n", __func__);
		goto exit_setup_pin;
	}
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - chip id check failed %d\n", __func__, ret);
		goto exit_chip_reset;
	}

	ret = sx9320_i2c_write(data, SX9320_SOFTRESET_REG, SX9320_SOFTRESET);
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - reset failed %d\n", __func__, ret);
		goto exit_chip_reset;
	}
	...
	ret = request_threaded_irq(data->irq, NULL, sx9320_interrupt_thread,
			...
			"sx9320_wifi_sub_irq", data);
	if (ret < 0) {
		pr_err("[SX9320_WIFI_SUB]: %s - failed to set request irq %d(%d)\n",
	...
	if (ret) {
		pr_err("[SX9320_WIFI_SUB] %s - cound not register sensor(%d).\n",
			__func__, ret);
...
#endif
	pr_info("[SX9320_WIFI_SUB]: %s - Probe done!\n", __func__);
...
exit_kzalloc:
exit:
	pr_err("[SX9320_WIFI_SUB]: %s - Probe fail!\n", __func__);
	return ret;
}
...
static int __init SX9320_wifi_sub_init(void)
{
	return i2c_add_driver(&sx9320_driver);
}

static void __exit SX9320_wifi_sub_exit(void)
{
	i2c_del_driver(&sx9320_driver);
}

module_init(SX9320_wifi_sub_init);
module_exit(SX9320_wifi_sub_exit);

MODULE_DESCRIPTION("Semtech Corp. SX9320 Capacitive Touch Controller Driver for wifi sub");
