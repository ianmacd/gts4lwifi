	...
	const struct i2c_device_id *id)
{
	...
	/* vio control for hw id 2 */
	if (system_rev == 2) {
		gpio_set_value(data->pdata->vio_ldo, 1);
		msleep(20);
	} else {
	...
	*/
	...
	if (system_rev == 2) {
		gpio_set_value(data->pdata->vio_ldo, 0);
		msleep(20);
		gpio_free(data->pdata->vio_ldo);
	}

	SENSOR_INFO("success system_rev = %d\n", system_rev);
	return ret;
