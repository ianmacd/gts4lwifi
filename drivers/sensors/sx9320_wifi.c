...
static int sx9320_check_hallic_state(char *file_path, char hall_ic_status[])
{
	...
	if (IS_ERR(filep)) {
		iRet = PTR_ERR(filep);
		pr_err("[SX9320_WIFI]: %s - file open fail %d\n", __func__, iRet);
		set_fs(old_fs);
		return iRet;
	}
	...
	if (iRet <= 0) {
		pr_err("[SX9320_WIFI]: %s - file read fail %d\n", __func__, iRet);
...
static void sx9320_send_event(struct sx9320_p *data, u8 state)
{
	...
	if (data->skip_data == true) {
		pr_info("[SX9320_WIFI]: %s - skip grip event\n", __func__);
		return;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (idx = 0; idx < (int)(sizeof(setup_reg) >> 1); idx++) {
		...
		pr_info("[SX9320_WIFI]: %s - Read Reg: 0x%2x Value: 0x%2x\n\n",
			__func__, setup_reg[idx].reg, val);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI]: %s - kstrtou8 failed.(%d)\n", __func__, ret);
		return count;
	}
	...
	if (onoff == 0) {
		...
		pr_err("[SX9320_WIFI]: %s - unknown value %d\n", __func__, onoff);
	}

	mutex_unlock(&data->read_mutex);

	pr_info("[SX9320_WIFI]: %s - %d\n", __func__, onoff);
...
static void sx9320_debug_work_func(struct work_struct *work)
{
	...
	if (strcmp(data->hall_ic, "CLOSE") == 0) {
		if (hall_flag) {
			pr_info("[SX9320_WIFI]: %s - hall IC is closed\n", __func__);
			sx9320_set_offset_calibration(data);
			hall_flag = 0;
		}
	} else {
...
static int sx9320_parse_dt(struct sx9320_p *data, struct device *dev)
{
	...
	if (sx9320_read_setupreg(node, SX9320_NORMALTHD_TA,
				&data->normal_th_ta)) {
		pr_info("[SX9320_WIFI]: %s - dt has no TA thd \n", __func__);
		data->normal_th_ta = data->normal_th;
	}

	pr_info("[SX9320_WIFI]: afeph0:%x, afeph1:%x, afeph2:%x, afeph3:%x\n",
		...
		unsigned long action, void *data)
{
	...
	pr_info("[SX9320_WIFI]: %s - src = %d, dest = %d, id = %d, attach = %d, drp = %d\n",
	...
	if (pdata->init_done == ON) {
		...
		case USB_STATUS_NOTIFY_DETACH:
			...
			break;
		default:
			pr_info("[SX9320_WIFI]: %s - DRP type : %d\n",
		...
		pr_info("[SX9320_WIFI]: %s - has not initialized\n", __func__);
	}
				...
				unsigned long action, void *data)
{
	...
	case ATTACHED_DEV_AFC_CHARGER_9V_MUIC:
		if (action == MUIC_NOTIFY_CMD_ATTACH)
			pr_info("[SX9320_WIFI]: %s - TA/USB insert\n", __func__);
		else if (action == MUIC_NOTIFY_CMD_DETACH)
			pr_info("[SX9320_WIFI]: %s - TA/USB remove\n", __func__);
...
static int sx9320_check_chip_id(struct sx9320_p *data)
{
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI]: whoami[0x%x] read failed %d\n", value, ret);
		return ret;
	}
	...
	case 0x21:
		return 0;
	default:
		pr_err("[SX9320_WIFI]: invalid whoami(%x)\n", value);
		return -1;
	}
}
		...
		const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		pr_err("[SX9320_WIFI]: %s - chip id check failed %d\n", __func__, ret);
