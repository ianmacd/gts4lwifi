...
static int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	switch (pSmart->panel_revision) {
		...
		case 'E' ... 'L':
			...
			break;
		case 'M':
			memcpy(base_luminance, base_luminance_revM, sizeof(base_luminance_revM));
			break;
		default: /* Rev.M ~ */
			memcpy(base_luminance, base_luminance_revM, sizeof(base_luminance_revM));
			break;
...
static int get_gradation_offset(int table_index, int index, int panel_revision)
{
	...
	switch (panel_revision) {
		...
		case 'E' ... 'L':
			...
			break;
		case 'M':
			ret = gradation_offset_revM[table_index][index];
			break;
		default: /* Rev.M ~ */
			ret = gradation_offset_revM[table_index][index];
			break;
...
static int get_rgb_offset(int table_index, int index, int panel_revision)
{
	...
	switch (panel_revision) {
		...
		case 'E' ... 'L':
			...
			break;
		case 'M':
			ret = rgb_offset_revM[table_index][index];
			break;
		default: /* Rev.M ~ */
			ret = rgb_offset_revM[table_index][index];
			break;
...
static int get_base_luminance_hmt(int brightness_level, int panel_revision)
{
	...
	switch (panel_revision) {
		...
		case 'E' ... 'L':
			...
			break;
		case 'M':
			memcpy(base_luminance, base_luminance_reverse_hmt_single_revM, sizeof(base_luminance_reverse_hmt_single_revM));
			break;
		default: /* Rev.M ~ */
			memcpy(base_luminance, base_luminance_reverse_hmt_single_revM, sizeof(base_luminance_reverse_hmt_single_revM));
			break;
...
static int get_gradation_offset_hmt(int table_index, int index, int panel_revision)
{
	switch (panel_revision) {
		...
		case 'M':
			return gradation_offset_reverse_hmt_single_revM[table_index][index];
		default: /* Rev.M ~ */
			return gradation_offset_reverse_hmt_single_revM[table_index][index];
	}
}
...
static int get_rgb_offset_hmt(int table_index, int index, int panel_revision)
{
	switch (panel_revision) {
		...
		case 'M':
			return rgb_offset_reverse_hmt_single_revM[table_index][index];
		default: /* Rev.M ~ */
			return rgb_offset_reverse_hmt_single_revM[table_index][index];
	}
}
