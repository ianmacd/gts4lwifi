...
static int ss_dsi_set_active_clock_info(struct ioctl_active_clock *ioctl_info)
{
	...
	ret = ss_dsi_update_clock_contol_cmd(1);
	if (ret)
		LCD_ERR("Failed to update clock control cmd\n");

	ret = mdss_samsung_send_cmd(ctrl, TX_ACT_CLOCK_ENABLE);
	if (ret)
		LCD_ERR("Failed to send clock control cmd\n");

	msleep(35);

	ret = ss_dsi_update_clock_contol_cmd(0);
	if (ret)
		LCD_ERR("Failed to update clock control cmd\n");
...
int ss_dsi_update_clock_contol_cmd(int time_update)
{
	char *pload = NULL;
	char *pload_d = NULL;
	struct act_clk_info *c_info;
	...
	b_info = &vdd->act_clock.blink_info;
	d_info = &vdd->act_clock.draw_info;
	pinfo = &vdd->ctrl_dsi[DISPLAY_1]->panel_data.panel_info;
	...
	if (c_info->en) {
		...
		if (b_info->pos1_x > pinfo->xres)
			b_info->pos1_x = pinfo->xres;
		if (b_info->pos1_y > pinfo->yres)
			b_info->pos1_y = pinfo->yres;
		...
		if (b_info->pos2_x > pinfo->xres)
			b_info->pos2_x = pinfo->xres;
		if (b_info->pos2_x > pinfo->xres)
			b_info->pos2_x = pinfo->xres;
...
int ss_dsi_update_clock_image_to_cmd(void)
{
	short copy_size=0;
	...
	for (loop=0 ; loop < ARRAY_SIZE(DSI_CMD_ACTIVE_CLK_IMG_TO_CMD) ; loop++) {
		DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].dchdr.dtype = DTYPE_GEN_LWRITE;
		DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].dchdr.last = 1;
		DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].payload = active_clk_img_to_cmd[loop];
		DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].dchdr.dlen = ACT_CLK_IMG_PAYLOAD_SIZE+1;
		p_load = DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].payload;
		copy_size = DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].dchdr.dlen-1;
		...
		if (cursor+copy_size > img_buf_size) {
			...
			DSI_CMD_ACTIVE_CLK_IMG_TO_CMD[loop].dchdr.dlen = copy_size+1;
		}
...
static int __init ss_dsi_active_clk_init(void)
{
	int ret = 0;
	struct samsung_display_driver_data *vdd = samsung_get_vdd();
	...
	if (!vdd->act_clock.is_support) {
		LCD_ERR("Not Support Active Clock Function \n");
		return -ENODEV;
	}
	...
	vdd->act_clock.act_clock_img_data_tx_cmds.link_state = DSI_HS_MODE;
	vdd->act_clock.act_clock_img_data_tx_cmds.cmds = DSI_CMD_ACTIVE_CLK_IMG_TO_CMD;
	vdd->act_clock.act_clock_img_data_tx_cmds.cmd_cnt = ARRAY_SIZE(DSI_CMD_ACTIVE_CLK_IMG_TO_CMD);
	vdd->act_clock.act_clock_control_tx_cmds.cmds = DSI_CMD_ACTIVE_CLK_CONTROL_CMD;
	vdd->act_clock.act_clock_control_tx_cmds.cmd_cnt = ARRAY_SIZE(DSI_CMD_ACTIVE_CLK_CONTROL_CMD);
	vdd->act_clock.act_clock_disable_tx_cmds.cmds = DSI_CMD_ACTIVE_CLK_CONTROL_DISABLE_CMD;
	vdd->act_clock.act_clock_disable_tx_cmds.cmd_cnt = ARRAY_SIZE(DSI_CMD_ACTIVE_CLK_CONTROL_DISABLE_CMD);
...
module_init(ss_dsi_active_clk_init);

static void __exit ss_dsi_active_clk_remove(void)
{
	struct samsung_display_driver_data *vdd = samsung_get_vdd();
	LCD_ERR("Active_Clock Remove \n");
	misc_deregister(&vdd->act_clock.dev);
}
module_exit(ss_dsi_active_clk_remove);

MODULE_DESCRIPTION("Active Clock driver");
MODULE_LICENSE("GPL");
