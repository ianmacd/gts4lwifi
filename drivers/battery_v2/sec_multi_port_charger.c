...
static void sec_multi_chg_set_input_current(struct sec_multi_charger_info *charger)
{
	...
	if (charger->sub_is_charging) {
		if (charger->cable_type == SEC_BATTERY_CABLE_POGO_WCIN) {
			...
			sub_input_current = sub_input_current - main_input_current;
...
static void calculate_charging_current(struct sec_multi_charger_info *charger, int *main_current, int *sub_current)
{
	int main_charging_current = *main_current;
	int sub_charging_current = *sub_current;
	int total_current = main_charging_current + sub_charging_current;
	if (charger->cable_type == SEC_BATTERY_CABLE_POGO_WCIN) {
		pr_info("%s: main curent: %d, main input: %d, sub input: %d\n",
				__func__, main_charging_current, charger->main_input_current, charger->sub_input_current);
		if (charger->main_input_current == 0) {
			main_charging_current = 0;
			sub_charging_current = total_current > charger->pdata->max_sub_charging_current ?
				charger->pdata->max_sub_charging_current : total_current;
		}
		else {
			if (main_charging_current > charger->sub_input_current)
				sub_charging_current = charger->sub_input_current;
			else
				sub_charging_current = main_charging_current;
			main_charging_current = main_charging_current - sub_charging_current;
		}
		pr_info("%s:  main current: %d, sub current: %d\n",
		...
		if (main_charging_current >= (total_current / 2))
			main_charging_current = (total_current / 2);
		sub_charging_current = total_current - main_charging_current;
		if (sub_charging_current > charger->pdata->max_sub_charging_current)
			sub_charging_current = charger->pdata->max_sub_charging_current;
	}
	pr_info("%s: total current: %d, main current: %d, sub current: %d\n",
			__func__, total_current, main_charging_current, sub_charging_current);
	*main_current = main_charging_current;
	*sub_current = sub_charging_current;
}
...
static void sec_multi_chg_set_charging_current(struct sec_multi_charger_info *charger)
{
	...
	mutex_lock(&charger->charger_mutex);
	if (charger->sub_is_charging) {
		calculate_charging_current(charger, &main_charging_current, &sub_charging_current);
	} else {
