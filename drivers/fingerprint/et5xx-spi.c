...
static int etspi_open(struct inode *inode, struct file *filp)
{
	...
	if (status == 0) {
		if (etspi->buf == NULL) {
			etspi->buf = kmalloc(bufsiz, GFP_KERNEL | GFP_DMA);
			if (etspi->buf == NULL) {
...
int etspi_platformInit(struct etspi_data *etspi)
{
	...
	/* gpio setting for ldo, sleep, drdy pin */
	if (etspi != NULL) {
		etspi->drdy_irq_flag = DRDY_IRQ_DISABLE;
		...
		if (etspi->buckenPin) {
			...
			if (status < 0) {
				pr_err("%s gpio_request etspi_bucken failed\n",
					__func__);
				goto etspi_platformInit_bucken_failed;
			}
			gpio_direction_output(etspi->buckenPin, 0);
		}
		if (etspi->ldo_pin) {
	...
	struct etspi_data *data)
{
	...
	if (of_property_read_u32(np, "etspi-wog", &data->detect_mode))
...
static void etspi_work_func_debug(struct work_struct *work)
{
	pr_info("%s ldo: %d, sleep: %d, tz: %d, type: %s\n",
		__func__, gpio_get_value(g_data->ldo_pin),
		gpio_get_value(g_data->sleepPin), g_data->tz_mode,
		sensor_status[g_data->sensortype + 1]);
}
...
static int etspi_set_timer(struct etspi_data *etspi)
{
	...
	setup_timer(&etspi->dbg_timer, etspi_timer_func, (unsigned long)etspi);
	etspi->wq_dbg =	create_singlethread_workqueue("etspi_debug_wq");
	if (!etspi->wq_dbg) {
		status = -ENOMEM;
...
static int __init etspi_init(void)
{
	...
	pr_info("%s\n", __func__);
		...
		&& defined(CONFIG_SENSORS_VFS8XXX)
	/* vendor check */
