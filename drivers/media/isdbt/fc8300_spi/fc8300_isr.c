...
static u8 ts_buffer[TS0_32PKT_LENGTH + 4];
#else
...
{
	if (buf_int_status) {
		bbm_data(handle, devid, BBM_TS0_DATA, &ts_buffer[0], g_pkt_length);
...
void fc8300_isr(HANDLE handle)
{
	...
	for(i = 0; i < 10; i++) {
		...
		if (buf_int_status) {
			...
			pr_debug("[FC8300] buf_int_status read count  %d\n", i+1);
		} else {
			break;
		}
	}
