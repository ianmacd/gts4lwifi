...
static int fc8350_spi_probe(struct spi_device *spi)
{
	...
	ISDB_PR_ERR("fc8350_spi_probe spi : 0x%p\n", spi);

	if (spi == NULL) {
		ISDB_PR_ERR("fc8350_spi_probe spi == NULL\n");
		return -1;
	}
	...
	if (ret < 0) {
		ISDB_PR_ERR("fc8350_spi_probe ERROR ret =%d\n", ret);
		return ret;
	}
		...
		u16 addr, u8 command, u8 *data, u16 length)
{
	...
	if (res) {
		ISDB_PR_ERR("fc8350_spi_bulkread fail : %d\n", res);
		return BBM_NOK;
	}
		...
		u16 addr, u8 command, u8 *data, u16 length)
{
	...
	for (i = 0; i < length; i++) {
		if ((4+i) < TX_DATA_SIZE)
			...
			ISDB_PR_ERR("Error spi_bulkwrite  tx_length= %d\n"
				, length);
	}
	res = fc8350_spi_write_then_read(fc8350_spi
	...
	if (res) {
		ISDB_PR_ERR("fc8350_spi_bulkwrite fail : %d\n", res);
		return BBM_NOK;
	}
		...
		u16 addr, u8 command, u8 *data, u32 length)
{
	...
	if (res) {
		ISDB_PR_ERR("fc8350 spi_dataread fail : %d\n", res);
		return BBM_NOK;
	}
