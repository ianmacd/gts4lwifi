...
static int sec_ts_check_firmware_version(struct sec_ts_data *ts, const u8 *fw_info)
{
	...
	if ((ts->plat_data->img_version_of_ic[0] != ts->plat_data->img_version_of_bin[0]) ||
		...
		input_info(true, &ts->client->dev, "%s: do not matched version info\n", __func__);
		return 0;
	}

	// temp code for rare case: IC will be flashed as SE00FFFF, and it can be normal used
...
static bool sec_ts_limited_flashpagewrite(struct sec_ts_data *ts, u32 page_idx, u8* page_data)
{
	...
	memcpy(&copy_data[2], page_data, SEC_TS_FLASH_SIZE_256);	/* DATA */
...
static int sec_ts_chunk_update(struct sec_ts_data *ts, u32 addr, u32 size, u8 *data)
{
	...
	mem_rb = (u8 *)kzalloc(fw_size, GFP_KERNEL | GFP_DMA);
	if (!mem_rb) {
...
static int sec_ts_firmware_update(struct sec_ts_data *ts, const u8 *data, size_t size, int bl_update)
{
	...
	temp_system_mode = ts->system_mode;
	ts->system_mode = TO_FLASH_MODE;
...
#elif defined(CALIBRATION_BY_SDC)
		input_info(true, &ts->client->dev, "%s: DO NOT CALIBRATION(cnt:%d, result:0x%x)\n", __func__, ts->cal_count, ts->cal_status);
#else
...
EXIT:
	ts->system_mode = temp_system_mode;
